      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="font-black text-[#4A2600] text-sm uppercase tracking-wider">Order Items</h3>
            <div className="bg-orange-50/50 rounded-xl p-3 space-y-3">
              {orderRows.map((item) => (
                <div key={item.id} className="flex items-center justify-between text-sm">
                  ...
                  <p className="font-black text-[#A03F00]">฿{item.subtotal.toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-2">
              ...
              <div className="bg-white border border-orange-100 rounded-xl p-3">
                <p className="font-bold text-[#4A2600] text-sm">{addressName || "Home Address"}</p>
                <p className="text-gray-600 text-xs leading-relaxed">{addressDetail}</p>
              </div>
            </div>
