            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-orange-100/50 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-orange-50/50 rounded-3xl p-6 border-2 border-orange-50 space-y-3">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] mb-1">Pickup Point</p>
                    <p className="text-sm font-bold text-[#4A2600] line-clamp-2">
                      {typeof service.pickup_address === "object"
                        ? `${service.pickup_address?.name || ""} ${service.pickup_address?.address_detail || ""}`.trim() || "Location shared after booking"
                        : service.pickup_address || service.detail_1 || "Location shared after booking"}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-orange-50/50 rounded-3xl p-6 border-2 border-orange-50 space-y-3">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] mb-1">Destination</p>
                    <p className="text-sm font-bold text-[#4A2600] line-clamp-2">
                      {typeof service.dest_address === "object"
                        ? `${service.dest_address?.name || ""} ${service.dest_address?.address_detail || ""}`.trim() || "Location shared after booking"
                        : service.dest_address || service.detail_2 || "Location shared after booking"}
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    pendingHireRequests.map((request) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-gray-500 line-clamp-1 italic">
                              "{request.request_message || "Hi, I'd like to hire you!"}"
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {decliningRequestRoomId === request.room_id ? "..." : "Decline"}
                          </button>
                          <button
                          ...
                          >
                            {acceptingRequestRoomId === request.room_id ? "Accepting..." : "Accept Job"}
                          </button>
                        </div>
                      </div>
                    ))}
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-orange-100 border-4 border-[#FF914D] sticky top-28 overflow-hidden">
              ...
              <div className="space-y-6 relative">
                <div>
                  <p className="text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] mb-1">Service Fee</p>
                ...
                <div className="space-y-3">
                  {hasActiveOrder ? (
                    ...
                    >
                      {startingChat ? <Loading fullScreen={false} size={20} /> : <MessageSquare className="w-5 h-5" />}
                    ...
                    <>
                      {canTryHire && !isHireRequested && (
                        <div className="space-y-4">
                          <textarea
                            ...
                            onChange={(e) => setHireRequestMessage(e.target.value)}
                          ...
                          >
                            {sendingHireRequest ? <Loading fullScreen={false} size={20} /> : <CheckCircle className="w-5 h-5" />}
                            {sendingHireRequest ? "Sending..." : "Request to Hire"}
                          </button>
                        </div>
                      )}
                      ...
                      {canTryHire && hasPendingHire && (
                        <div className="space-y-3">
                          ...
                          >
                            {cancelingHireRequest ? "Canceling..." : "Cancel My Request"}
                          </button>
                        </div>
                      )}
                    </>
                ...
                <div className="pt-6 border-t border-orange-50">
                   <p className="text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] mb-4">Service Provider</p>
                   <div className="flex items-center gap-4 bg-orange-50/30 p-4 rounded-3xl border-2 border-orange-50">
                      <div className="w-14 h-14 rounded-2xl bg-white border-2 border-white overflow-hidden flex items-center justify-center shadow-md shadow-orange-100">
                        {creator?.avatar_url ? (
                          <img src={creator.avatar_url} className="w-full h-full object-cover" alt={creator.full_name || ""} />
                        ) : (
                ...
                {isFreelancer && !isServiceOwner && !hasActiveOrder && (
                  <div className="bg-blue-50 border-2 border-blue-100 rounded-2xl p-4 flex gap-3">
                    ...
                    <p className="text-[10px] font-bold text-blue-700 leading-relaxed">
                      You are logged in as a Freelancer. Only Customers can request to hire services.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-green-50/50 rounded-[2.5rem] p-8 border-2 border-green-50 space-y-4">
               ...
               <ul className="space-y-3">
                 {["Verify location before booking", "Use in-app chat for coordination", "Release payment after completion"].map((tip, i) => (
                   <li key={i} className="flex items-start gap-2">
                     <div className="w-1.5 h-1.5 rounded-full bg-green-300 mt-1.5 shrink-0" />
                     <p className="text-[11px] font-bold text-green-700/80">{tip}</p>
                   </li>
                 ))}
               </ul>
            </div>
